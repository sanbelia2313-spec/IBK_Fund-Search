...
function cleanFundCharParagraph(str) {
  ...
  // (2026-08-04 수정) 이 폰트는 글머리 기호(원래 ▪ 같은 도형)를 유니코드 "개인 사용 영역"
  // 문자(예: \uf0a7)로 인코딩해놔서, 텍스트로 뽑으면 화면에 안 보이는 빈칸처럼 나온다.
  // 눈에 보이는 "•"로 바꿔서 각 항목이 구분되게 함(뒤이어 나오는 마커 줄바꿈 규칙에서
  // "•"도 새 줄로 끊어주는 대상에 포함시킴).
  s = s.replace(/[\uf0a7\uf0b7\uf06f\uf02d\u2022]/g, "•");
  // (2026-08-04 수정) "1. 투자목적", "2. 투자전략"처럼 마침표+공백으로 된 대분류 번호
  // 소제목 앞에는 줄바꿈이 안 들어가서, 앞 문단 마지막 문장에 "2. 투자전략"이 그대로
  // 붙어버리는 문제가 있었다("...공시할 예정입니다. 2. 투자전략 가. 기본 운용..."). 소수점
  // 숫자(예: "97.5%")는 마침표 뒤에 공백 없이 바로 숫자가 오므로 이 패턴과 겹치지 않아
  // 안전하게 구분됨. 같이 "<모투자신탁의 포트폴리오 구성>" 같은 꺾쇠 소제목("<" 바로 뒤에
  // 한글이 오는 경우만 — 숫자 비교식 "10<20" 등과 헷갈리지 않도록)과, 위에서 통일한 "•"
  // 글머리 기호도 매번 새 줄로 끊어준다.
  s = s.replace(/\s*(?=(?:\d{1,2}\)|①|②|③|④|⑤|⑥|※|\d{1,2}\.\s[가-힣]|<(?=[가-힣])|•))/g, "\n").trim();
...
function findInvestStrategyFromSummaryBox(text) {
  ...
  // (2026-08-04 수정) 아래 토큰 제거 로직은 "표 라벨 파편"(예: 본문 줄 앞뒤에 섞여 붙는
  // "투자목적"/"및"/"전략")만 지우려는 의도인데, 실제로는 "2. 투자전략"이나 "가. 기본 운용
  // 전략"처럼 문서 본문에 진짜로 있는 소제목의 끝 단어가 우연히 "전략"/"투자전략"과 같아서
  // 소제목 전체가 반쪼가리로 잘려나가는 문제가 있었다(IBK자산운용 실제 PDF로 확인: "2. 투자전략"
  // → "2."만 남고, "가. 기본 운용 전략" → "가. 기본 운용"만 남음). 라벨 파편은 "숫자." 나
  // "가./나./다."로 시작하는 소제목 형태로 나타나지 않으므로, 그런 줄은 소제목으로 보고
  // 토큰 제거를 건너뛴다(원문 그대로 보존).
  const HEADING_LINE_RE = /^\s*(?:\d{1,2}\s*\.|[가-힣]\s*\.)/;
  function stripLabelEdgeTokens(line) {
    if (HEADING_LINE_RE.test(line)) return line.trim();
